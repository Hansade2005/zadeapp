...
export const ContactOrganizerModal: React.FC<ContactOrganizerModalProps> = ({
  ...
  const { user } = useAuth();
  const [subject, setSubject] = useState(`Inquiry about: ${eventTitle}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      toast.error('Please login to contact the organizer');
      return;
    }
      ...
      <div className="bg-white rounded-lg max-w-md w-full">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xl font-bold text-gray-900">Contact Organizer</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-500">About: {eventTitle}</p>
          </div>
